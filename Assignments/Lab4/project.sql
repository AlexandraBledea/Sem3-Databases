...
CREATE TABLE Borrowed (
	...
	foreign key(CNP_Sub) references Subscriber(ID) on delete cascade on update cascade,
...
CREATE TABLE Subscription (
	...
	Availability BIT,
	foreign key (Library_ID) references Library(ID) on delete cascade on update cascade,
	foreign key (ID_Sub) references Subscriber(ID) on delete cascade on update cascade,
	constraint pk_subscription primary key (Library_ID, ID_Sub)
);
...
CREATE TABLE Review (
	...
	Grade integer,
	foreign key (Book_ID) references Book(ID) on delete cascade on update cascade,
	foreign key (CriticID) references Critic(ID) on delete cascade on update cascade,
	constraint pk_review primary key (Book_ID, CriticID)
);
